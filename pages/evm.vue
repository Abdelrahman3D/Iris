<template lang="pug">
    ...
    input#exa(type='range' ref="exa" style='width: 700px', min='0', max='5', step='0.01', oninput='updateParams')
    br
    label(for='alpha') Alpha: 
    input#alpha(type='range' ref="alpha" style='width: 700px', min='1', max='100', step='0.01', oninput='updateParams')
    br
    label(for='lambdac') Lambda: 
    input#lambdac(type='range' ref="lambdac" style='width: 700px', min='1', max='90', step='0.01', oninput='updateParams')
    br
    label(for='chroma') Chroma: 
    input#chroma(type='range', ref="chroma" style='width: 700px', min='0', max='10', step='0.01', oninput='updateParams')
    br
    label(for='r1') r1: 
    input#r1(type='range' ref="r1" style='width: 700px', min='0', max='1', step='0.01', oninput='updateParams')
    br
    label(for='r2') r2: 
    input#r2(type='range', ref="r2" style='width: 700px', min='0', max='1', step='0.01', oninput='updateParams')
    br
    video#webcam
    canvas#canvas
    canvas#savnac
...
export default {
  ...
  methods: {
    prepare (width, height) {
      ...
      height = Math.floor(height * s >> 3) << 3;
      this.$refs.video.style.width = `${width}px`;
      width = Math.max(480, width);
      this.$refs.savnac.width = this.$refs.canvas.width = width;
      this.$refs.savnac.height = this.$refs.canvas.height = height;
    ...
    render () {
      ...
      this.ctx.drawImage(this.$refs.video, 0, 0, this.$refs.video.videoWidth * s, this.$refs.video.videoHeight * s)
